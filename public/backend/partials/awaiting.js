...
$(document).ready(function(){
    ...
    var table = $('#awaiting').DataTable({
      ...
      order: [0, 'desc'],
      "ajax" :{
        'url' : baseUrl+'/getAwaitingApprovalBlog',
        'type' : 'GET',
      },
      columns: [
      {data: 'id', name: 'id'},
...

      {
        "render" : function (data, type, row, meta)
        {
          return `<a href="${baseUrl}/editBlog/${row.id}" class="btn btn-primary btn-sm btn-circle"><i class='fas fa-pencil-alt'></i></a>`;
        },
        "targets" : 7
      },
      ...
      },
      {
        "render" : function (data, type, row, meta)
        {
          return `<input type="checkbox" class="approve" name="approve" id="${row.id}">`;
        },
        "targets" : 9
      },
      ]
